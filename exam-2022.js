...
const STORAGE_KEY='digestive-exam-2022-v1';
...
function backupAnswers(){
 ...
 const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='2022-실전풀이-답안.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
...
function updateNavigation(){
 buttons.forEach((b,i)=>{
  ...
  b.classList.toggle('answered',answered(q));b.classList.toggle('review-correct',!!state.submittedAt&&!q.ungradable&&correct);b.classList.toggle('review-incorrect',!!state.submittedAt&&!q.ungradable&&!correct);
  b.classList.toggle('flagged',state.flags.includes(q.id));b.setAttribute('aria-label',`${i+1}번 문항, ${state.submittedAt?(q.ungradable?'채점 제외':correct?'정답':'오답'):(answered(q)?'응답':'미응답')}`);
...
function renderChoices(q){
 ...
 q.choices.forEach((text,i)=>{
  ...
  if(state.submittedAt&&!q.ungradable&&input.checked){const correct=q.allowManual&&typeof state.overrides[q.id]==='boolean'?state.overrides[q.id]:q.answerIndices.includes(i+1);label.classList.add(correct?'review-correct':'review-incorrect');const icon=document.createElement('span');icon.className='correctness-icon';icon.textContent=correct?'✓':'✕';icon.setAttribute('aria-label',correct?'정답 선지':'오답 선지');content.append(icon);}
...
function render(){
 ...
 el('question-number').textContent=state.index+1;el('question-points').textContent=q.ungradable?'채점 제외':`총 ${q.points.toFixed(2)}점`;
 ...
 el('review-answer').hidden=!review;el('review-answer').textContent=review?(q.ungradable?'정답 확정 불가 · 채점 제외':q.choices.length?`${q.allowManual?'추정 정답':'정답'}: ${q.answerIndices.map(n=>`${n}. ${q.choices[n-1]}`).join(' / ')}`:`모범답안\n${q.modelAnswer}`):'';
...
function updateResults(){
 ...
 el('final-score').textContent=`이번 퀴즈의 최종 점수는 ${score.earned.toFixed(2)}/${score.total.toFixed(2)} 입니다. (복원 불충분 ${questions.filter(q=>q.ungradable).length}문항 채점 제외)`;
}
