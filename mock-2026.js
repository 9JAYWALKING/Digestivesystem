...
const STORAGE_KEY='digestive-mock-2026-original-v4';
...
const signature=data.version+'|'+questions.map(q=>q.id).join('|');
let saved=null;
try{saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null')}catch{}
// Preserve an existing attempt when only choice order / distractors have changed.
if(saved&&saved.signature!==signature){
 const migration=(data.migrations||[]).find(m=>m.fromSignature===saved.signature);
 if(migration){
  const answers={...saved.answers};
  for(const q of questions){
   const old=answers[q.id],map=migration.optionMaps[q.id];
   if(Array.isArray(old)&&Array.isArray(map))answers[q.id]=old.map(n=>map[n-1]).filter(n=>Number.isInteger(n)&&n>0);
  }
  saved={...saved,answers,signature};
 }
}
...
if(saved&&saved.signature===signature){
 ...
 for(const q of questions){
  ...
  if(!q.choices.length&&typeof saved.overrides?.[q.id]==='boolean')state.overrides[q.id]=saved.overrides[q.id];
 }
...
function backupAnswers(){
 ...
 const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='2026-모의고사-답안.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
...
function renderChoices(q){
 ...
 q.choices.forEach((text,i)=>{
  ...
  if(state.submittedAt&&input.checked){const correct=q.answerIndices.includes(i+1);label.classList.add(correct?'review-correct':'review-incorrect');const icon=document.createElement('span');icon.className='correctness-icon';icon.textContent=correct?'✓':'✕';icon.setAttribute('aria-label',correct?'정답 선지':'오답 선지');content.append(icon);}
...
function render(){
 ...
 (q.images||[]).forEach((src,i)=>{
  ...
  img.addEventListener('error',()=>{const warning=document.createElement('p');warning.className='image-error';warning.textContent='이미지를 불러오지 못했습니다. 연결 상태를 확인하세요.';img.replaceWith(warning);},{once:true});
  const label=q.imageLabels?.[i];
  if(label){
   const figure=document.createElement('figure');figure.setAttribute('style','margin:0;flex:1 1 260px;min-width:0;max-width:100%;text-align:center');
   const caption=document.createElement('figcaption');caption.textContent=label;caption.setAttribute('style','font-weight:700;margin-bottom:6px');
   img.alt=`문제 ${state.index+1} 사진 ${label}`;img.setAttribute('style','margin:0 auto');
   figure.append(caption,img);el('question-images').append(figure);
  }else el('question-images').append(img);
 });
 ...
 el('review-answer').hidden=!review;el('review-answer').textContent=review?(q.choices.length?`정답: ${q.answerIndices.map(n=>`${n}. ${q.choices[n-1]}`).join(' / ')}`:`모범답안\n${q.modelAnswer}`):'';
 el('exam-override-note').hidden=!review||!q.examOverride;
 el('grade-correction').hidden=!review||!!q.choices.length;
 if(review&&!q.choices.length){
  const overridden=typeof state.overrides[q.id]==='boolean';
  el('correction-status').textContent=`${overridden?'정정':'자동 채점'}: ${isCorrect(q)?'정답':'오답'} · ${isCorrect(q)?q.points.toFixed(2):'0.00'} / ${q.points.toFixed(2)}점`;
...
function correctGrade(value){
 const q=questions[state.index];if(!state.submittedAt||q.choices.length)return;
...
function escapeDetail(value){return String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
function loadDetail(){
 const q=questions[state.index];if(detailQuestion===q.id)return;
 detailQuestion=q.id;clearTimeout(detailTimeout);
 el('detail-title').textContent=`문제 ${state.index+1} · 해설`;
 const base=new URL('./index.html',location.href).href;
 el('detail-original').href=q.sourceNumber?`${base}#jbl-q-${q.sourceNumber}`:`${base}#lecture-${q.lectureId}`;
 el('detail-original').textContent=q.sourceNumber?(q.advancedVariant?'변형 근거 J 보기':'원본 J 보기'):q.hasWiki?'Wiki 보기':'강의 보기';
 el('detail-status').hidden=true;
 const answer=q.choices.length?q.answerIndices.map(n=>`${n}. ${q.choices[n-1]}`).join(' / '):q.modelAnswer;
 const references=q.sourceRefs.map(n=>`<a href="${escapeDetail(base)}#jbl-q-${n}" target="_blank" rel="noopener">J ${n}번</a>`).join(' ');
 const sourceSection=q.section?`<p class="note">학습 문단: ${escapeDetail(q.section)}</p>`:'';
 const wiki=q.hasWiki?`<a href="${escapeDetail(base)}#lecture-${q.lectureId}" target="_blank" rel="noopener">${q.lectureId===8?'8·9':q.lectureId}강 Wiki</a>`:'';
 const notice=q.outsideLecture?'출제자의 요청에 따라 강의록에 없는 내용을 출제하였다.':q.advancedVariant?'Wiki와 관련 기출을 대조하여 조건·판단 방향·선지를 새롭게 구성한 모의고사 문항입니다. 연결된 J의 정답과 이 문항의 정답은 다를 수 있습니다.':q.originalAuthored?(q.hasWiki?'Wiki의 학습 내용을 바탕으로 새로 출제한 모의고사 전용 문항입니다.':'해당 강의의 약물 범위에서 새로 출제하고 약물 안전성 근거를 확인한 모의고사 전용 문항입니다.'):q.reconstructed?'조건·선지 또는 사진을 모의고사용으로 보완했습니다. 기존 J 원문은 변경하지 않았습니다.':q.exact?'2025 원문 발문과 선지를 그대로 수록했습니다.':'기출의 출제 포인트를 유지한 변형 문항입니다.';
 el('detail-frame').title=`문제 ${state.index+1}의 모의고사 해설`;
 el('detail-frame').srcdoc=`<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;padding:24px;color:#254657;background:#fff;font:16px/1.8 'Malgun Gothic',sans-serif;overflow-wrap:anywhere}h1{font-size:20px;color:#005799;margin:0 0 18px}h2{font-size:16px;margin:24px 0 8px}p{white-space:pre-line}a{display:inline-block;color:#005799;font-weight:700;text-decoration:none;padding:5px 11px;margin:0 6px 6px 0;border:1px solid #b9d8e5;border-radius:6px}.answer{padding:15px;background:#e8f5fa;border-left:4px solid #005799}.note{font-size:13px;color:#75848b;border-top:1px solid #e1e8ec;padding-top:16px}</style><h1>문제 ${state.index+1} 해설</h1><p class="answer">정답: ${escapeDetail(answer)}</p><h2>정답을 가르는 근거</h2><p>${escapeDetail(q.explanation)}</p><h2>연결 학습</h2>${wiki} ${references}${sourceSection}<p class="note">${escapeDetail(notice)}</p></html>`;
}
